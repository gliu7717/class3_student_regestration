...
class StudentDatabase (val url:String, val user:String, val password: String ):  StudentDao{
    ...
    companion object {
        ...
        fun getInstance(context: Context): StudentDatabase? {
            synchronized(this) {
                if (INSTANCE == null) {
                    INSTANCE = StudentDatabase("jdbc:mysql://150.136.175.102:3306/android?characterEncoding=latin1&useConfigs=maxPerformance",
                                "gliu",
                                "benray1110")
                }
                return INSTANCE
    ...
    override suspend fun insertStudent(student: Student) {
        ...
        if (connection != null) {
            ...
            val sql = "insert into student(name, email) values('${student.name}', '${student.email}')"
    ...
    override suspend fun updateStudent(student: Student) {
        ...
        if (connection != null) {
            val statement = connection!!.createStatement()
            val sql = "update student set name = '${student.name}', email =  '${student.email}' where id = ${student.id}"
            statement.executeUpdate(sql)
        }
    }
    ...
    override suspend fun deleteStudent(student: Student) {
        ...
        if (connection != null) {
            val statement = connection!!.createStatement()
            val sql = "delete from student where id = ${student.id}"
            statement.executeUpdate(sql)
        }
    }
